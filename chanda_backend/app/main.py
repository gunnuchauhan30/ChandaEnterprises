...
app.add_middleware(
    ...
    # Belt-and-suspenders for Railway: every deploy (including PR/staging
    # deploys and the occasional service redeploy) gets a fresh
    # *.up.railway.app subdomain, and CORS_ORIGINS in .env is easy to forget
    # to update when that happens -- which shows up to users as a generic,
    # hard-to-diagnose "Network error" in the browser (CORS failures don't
    # give a helpful message; fetch() just throws). This regex covers any
    # Railway subdomain so the frontend always works without needing a env
    # var edit + redeploy every time its URL changes. Tighten this to your
    # exact production domain(s) via CORS_ORIGINS once the frontend URL is
    # stable, if you want to lock it down further.
