"""
SMTP email sender. Disabled by default (settings.EMAIL_ENABLED=False) so the
backend runs fully without SMTP creds configured. Flip EMAIL_ENABLED=True in
.env and fill SMTP_* once real credentials are available -- nothing else
needs to change.
"""
import smtplib
import logging
