...
from app.models import EmployeeRequest, Issue, Return, Material, User, StockBatch
from app.schemas.issues import (
    EmployeeRequestIn, EmployeeRequestOut, EmployeeRequestDecision,
    EmployeeRequestBulkIn,
    IssueIn, IssueOut, IssueConsumptionUpdate,
    ReturnIn, ReturnOut,
)
from app.services.numbering import next_request_no, next_issue_no, next_return_no, next_job_card_no
...
def list_requests(
    ...
    rows = q.order_by(EmployeeRequest.created_at.desc()).offset((page - 1) * page_size).limit(page_size).all()

    # Update 2: material name alongside the code, everywhere a request list
    # is rendered (Requests page, Backorders, etc.) -- one extra query per
    # page instead of N+1 per row.
    codes = {r.material_code for r in rows}
    names = {m.material_code: m.material_name for m in db.query(Material).filter(Material.material_code.in_(codes)).all()} \
        if codes else {}
    out = []
    for r in rows:
        item = EmployeeRequestOut.model_validate(r)
        item.material_name = names.get(r.material_code)
        out.append(item)
    return out
...
def create_request(
    ...
    out.material_name = material.material_name
...
def create_bulk_request(
    payload: EmployeeRequestBulkIn,
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user),
):
    """
    UPDATE 5-POINTS: Create multiple material requests at once.
    Each item becomes a separate request record, but all linked by department/job_card.
    Returns the first request_no as reference.
    """
    ...
    for item in payload.items:
        material = db.query(Material).get(item.material_code)
        if not material:
            raise HTTPException(404, f"Material {item.material_code} not found")
        ...
        log_activity(
            db, current_user.id,
            f"Raised material request {req.request_no} (bulk: {item.material_code} x {item.requested_qty})",
            "employee_requests"
        )
    
    # Return the first request details + count of all created
    if created_requests:
        first = created_requests[0]
        out = EmployeeRequestOut.model_validate(first)
        out.stock_warning = f"Created {len(created_requests)} requests in bulk starting from {first.request_no}"
        return out
    
    raise HTTPException(500, "Failed to create requests")


def _auto_lot_no(db: Session, material_code: str) -> str | None:
    """
    FIFO oldest batch with stock left -- the one that will actually get
    consumed for this issue -- so the Route Card always prints the lot
    that's genuinely going out the door. Returns None if there's no batch
    on record (e.g. very old stock predating batch tracking).
    """
    oldest_batch = (
        db.query(StockBatch)
        .filter(StockBatch.material_code == material_code, StockBatch.remaining_qty > 0)
        .order_by(StockBatch.received_date.asc(), StockBatch.id.asc())
        .first()
    )
    return oldest_batch.batch_no if oldest_batch else None
...
def _issue_against_request(db: Session, req: EmployeeRequest, fulfill_qty, issued_by_id: int) -> Issue:
    ...
    issue = Issue(
        ...
        job_card_no=req.job_card_no or next_job_card_no(db),
        lot_no=_auto_lot_no(db, req.material_code),
...
def decide_request(
    ...
    if payload.action == "approve":
        material = db.query(Material).get(req.material_code)
...
def list_backorders(
    ...
    rows = [r for r in rows if (r.requested_qty - (r.fulfilled_qty or 0)) > 0]
...
def list_issues(
    ...
    names = {u.id: (u.full_name or u.username) for u in db.query(User).filter(User.id.in_(issuer_ids)).all()} \
        if issuer_ids else {}
    codes = {r.material_code for r in rows}
    mat_names = {m.material_code: m.material_name for m in db.query(Material).filter(Material.material_code.in_(codes)).all()} \
        if codes else {}
    out = []
    for r in rows:
        item = IssueOut.model_validate(r)
        item.issued_by_name = names.get(r.issued_by)
        item.material_name = mat_names.get(r.material_code)  # Update 2
        out.append(item)
    return out
...
def create_direct_issue(
    ...
    if not payload_data.get("lot_no"):
        ...
        auto_lot = _auto_lot_no(db, payload.material_code)
        if auto_lot:
            payload_data["lot_no"] = auto_lot
    if not payload_data.get("job_card_no"):
        # Keeps every issue's Job Card No. populated (never blank), and in
        # step with the Route Card list's Sr. No. column when nobody typed
        # a real job card number by hand.
        payload_data["job_card_no"] = next_job_card_no(db)
...
def _material_description(material: Material) -> str:
    """
    Full human-readable description for a material: name + size + grade,
    whichever parts are actually filled in (many materials only have a
    size-like code in material_name and nothing in size/grade separately,
    so this never shows a blank or duplicate segment). Used anywhere a
    printed document needs to show "what material, what size" instead of
    just the bare material_name.
    """
    parts = []
    if material.material_name:
        parts.append(material.material_name)
    if material.size and material.size != material.material_name:
        parts.append(material.size)
    if material.grade:
        parts.append(material.grade)
    return " — ".join(parts) if parts else material.material_code
...
def _route_card_rows(db: Session, date_from, date_to, department, job_card_no, shift):
    ...
    for idx, i in enumerate(issues, start=1):
        # part_name falls back to a full material description (name + size
        # + grade) when no separate part_number was captured on the issue,
        # so the sheet actually shows what material/size went out, not just
        # a bare code.
        material = db.query(Material).get(i.material_code)
        part_name = i.part_number or (_material_description(material) if material else i.material_code)
        rows.append({
            "sr_no": idx,
