...
from app.services.numbering import next_grn_no
...
def create_grn(
    ...
    # Update 3: Lot No. is now the same number as the GRN it came in on
    # (e.g. GRN GRN-000042 -> Lot LOT-000042), instead of an independent
    # counter. This makes the GRN <-> Lot link direct and unambiguous --
    # given a lot on the shop floor, the GRN that brought it in is the same
    # number, no cross-referencing needed -- and since a GRN is always for
    # one material, this is automatically material-wise traceable too.
    # Whatever the frontend sent for batch_no is still ignored -- never
    # hand-typed.
    payload_data["batch_no"] = "LOT-" + grn_no.split("-", 1)[1] if "-" in grn_no else grn_no
    purchase = Purchase(
        grn_no=grn_no,
...
def update_qc_status(
    ...
    """Setting qc_status='passed' here is what actually triggers the automatic
    stock increase (DB trigger trg_purchase_stock), which in turn may flip a
