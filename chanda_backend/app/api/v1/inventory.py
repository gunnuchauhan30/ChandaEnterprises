...
from app.models import StockBatch, StockLedger, Material, User, StockReconciliation, Return
...
# is a thin wrapper around the exact same reconciliation -> adjustment ->
# stock_ledger trigger path used above, just created and auto-approved in
# one call instead of being left pending for a second admin to review.
# Every edit still lands in stock_reconciliations, returns (as an
# 'adjustment' row) and stock_ledger -- fully auditable, and the
# reconciliation row records exactly who made the change and when.
#
# Update 1: opened up to store_manager as well as admin (previously
# admin-only, with store managers routed through the slower two-step
# /reconciliations submit -> separate-admin-approval flow instead). Full
# Material Master access for Store now includes this inline stock
# correction, not just min/max and material edit.
