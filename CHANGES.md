...
## Latest Update: FIFO Issue Preview + Inventory Supplier Breakdown

**Problem found:** `issue-form.html` was already calling `POST /issues/fifo-check`,
but that route never existed on the backend (only a dead/broken
`fifo_service.py` existed, referencing a `StockLedger.supplier_id` column
that isn't even in the schema) — so the FIFO box on the Issue page silently
never worked. Inventory summary also had no real per-supplier data at all;
the frontend was already trying to group by `supplier_name` but the backend
never sent it.

**Fixed, using the real schema** (`stock_batches` → `purchases` →
`suppliers`, the same tables the DB trigger `fn_trg_issue_stock` already
uses for FIFO consumption):

- `POST /api/v1/issues/fifo-check` (new, read-only) — given a material +
  quantity, returns the oldest-batch-first supplier breakdown: Supplier →
  Available → Issue Qty → Remaining, plus totals and a shortfall/status flag.
  Nothing is written to the DB; it's a pure preview of what the real Issue
  will do.
- `GET /api/v1/inventory/summary` — now also returns a `suppliers[]` array
  per material (supplier name, remaining qty, oldest received date, FIFO
  order), alongside the existing material-level totals.
- `issue-form.html` — FIFO section rewritten as a connected flow (Supplier
  card → Available → Issue → Remaining, oldest at top), with a TOTAL row and
  a clear "can fully issue" / "short by X" status line.
- `inventory.html` — each material row now shows its supplier sub-rows
  (indented, FIFO order, with the date since which that stock has been
  sitting) and an explicit TOTAL row when more than one supplier is
  contributing stock to that material.
