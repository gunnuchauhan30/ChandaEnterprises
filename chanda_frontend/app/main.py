...
import os
import requests as http_requests  # aliased: this file also has a route function
                                    # named requests() (Material Requests page),
                                    # which would otherwise silently shadow the
                                    # 'requests' HTTP library at module scope.
...
# ============================================
# API PROXY (fixes "Network error" from the browser)
# ------------------------------------------------------------------
# The old setup had the BROWSER's JavaScript (api.js) call the backend's
# Railway URL directly. That URL is fragile -- every time the backend
# service gets redeployed / renamed on Railway, its public domain can
# change, and api.js's hardcoded fallback then points at a dead domain,
# producing exactly the "Network error -- is the backend running on ..."
# toast seen on the dashboard.
#
# Fix: the browser now always calls THIS Flask app, same-origin, at
# /api/v1/<path>. Flask (running server-side on Railway) forwards that
# request to BACKEND_URL and streams the response back. BACKEND_URL can
# even be Railway's private internal address (e.g.
# http://backend.railway.internal:8000) since this call happens
# server-to-server, not from the user's browser -- so the backend no
# longer needs a public domain at all, and there's nothing for the
# frontend's JS to get wrong.
# ============================================
...
def api_proxy(subpath):
    target_url = f"{BACKEND_URL}/api/v1/{subpath}"
    forward_headers = {
    ...
    try:
        ...
        return jsonify({
            "detail": f"Could not reach backend at {BACKEND_URL}. "
                      f"Check the BACKEND_URL environment variable on the frontend "
                      f"Railway service. Error: {str(e)}"
        }), 502
...
@app.route('/issues/fifo-preview')
@login_required
def issue_fifo_preview():
    """Dedicated FIFO confirmation page -- opens after 'Issue Material' is
    clicked on the New Issue form, before the issue is actually created.
    Shows exactly which supplier's stock (oldest date first) will be
    deducted, then confirms and creates the issue from here."""
    return render_template('pages/issue-fifo-preview.html', page='issues')
