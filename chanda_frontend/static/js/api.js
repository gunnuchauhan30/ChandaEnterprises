...
// Point: all requests go to THIS server (same origin), which proxies them
// to the backend server-side (see /api/v1/<path> route in chanda_frontend
// app/main.py). This means the browser never needs to know the backend's
// Railway URL directly -- so a backend redeploy/URL change can never again
// break the frontend with a "Network error" like it did before.
...
class API {
    ...
    static async getIssueFifoSource(issueId) {
        return this.request(`/issues/${issueId}/fifo-source`);
    }
    ...
    static async getStockSyncCheck() {
        return this.request('/inventory/stock-sync-check');
    }

    static async fixGhostStock() {
        return this.request('/inventory/fix-ghost-stock', { method: 'POST' });
    }

    static async undoGhostStockFix() {
        return this.request('/inventory/undo-ghost-stock-fix', { method: 'POST' });
    }

    static async fixPhantomBatches() {
        return this.request('/inventory/fix-phantom-batches', { method: 'POST' });
    }
    ...
    // used by the "Send Now" test button so someone doesn't have to wait
    // until 12:00 PM to confirm SMTP is configured correctly.
    static async sendInventorySummaryNow() {
        return this.request('/reports/inventory-summary/send-now', { method: 'POST' });
    }
